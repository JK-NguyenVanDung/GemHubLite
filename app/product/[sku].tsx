...
export default function ProductDetailScreen() {
  ...
  const saveProduct = async (patch: Parameters<typeof mutate>[0]) => {
  ...
  return (
    <Screen testID="product-detail-screen" scroll={false} safeAreaEdges={["left", "right"]} contentStyle={{ padding: 0, gap: 0 }}>
      ...
      <ScrollView keyboardShouldPersistTaps="handled"
        ...
        contentContainerStyle={{ alignSelf: "center", gap: layout.contentGap, maxWidth: layout.contentMaxWidth, padding: layout.pagePadding, paddingBottom: layout.pagePadding + theme.spacing.lg, width: "100%" }}
        scrollIndicatorInsets={{ bottom: theme.spacing.lg }}
      >
        ...
        <ProductFormSection key={`${product.sku}-${product.updatedAt}`} initialTitle={product.title} initialType={product.type} initialDescription={product.description} saving={saving} error={saveError} onDeleteProduct={confirmDeleteProduct} onSave={saveProduct} />
      </ScrollView>
    </Screen>
  );
}
